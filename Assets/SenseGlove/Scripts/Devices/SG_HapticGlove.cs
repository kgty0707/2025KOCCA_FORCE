    ...
    public class SG_HapticGlove : MonoBehaviour, IHandPoseProvider, IHandFeedbackDevice
    {
        ...
        public bool GetWristLocation(out Vector3 wristPos, out Quaternion wristRot)
        {
            ...
            else if (this.wristTrackingMethod == WristTracking.FollowObjectNoOffsets)
            {
                ...
                {
                    wristPos = this.transform.position;
                    wristRot = this.transform.rotation * Quaternion.Euler(0, -90, 0); // y축 회전 보정
                }
                return true;
            }
        ...
        protected virtual void LateUpdate()
        {
            ...
            // Update the hand model position and rotation based on wristTrackingObj
            if (this.wristTrackingMethod == WristTracking.FollowObjectNoOffsets && this.wristTrackingObj != null)
            {
                Vector3 wristPos;
                Quaternion wristRot;
                if (GetWristLocation(out wristPos, out wristRot))
                {
                    this.transform.position = wristPos;
                    this.transform.rotation = wristRot;
                }
            }
        }
